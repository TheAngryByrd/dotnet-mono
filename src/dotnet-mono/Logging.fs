...
module LoggerEx =
  ...
  type Logger with
    ...
    member x.logSync message : unit =
      logWithTimeout x message.level (fun _ -> message) |> Async.RunSynchronously
...
type LoggingConfig =
  { /// The `timestamp` function should preferably be monotonic and not 'jumpy'
    ...
    getLogger        : LogLevel*string[] -> Logger
...
module Global =
  ...
  let defaultConfig =
    { timestamp        = fun () -> DateTimeOffset.timestamp DateTimeOffset.UtcNow
      getLogger        = fun (level,name) -> LiterateConsoleTarget(name, level) :> Logger
  ...
  let internal getStaticLogger (level,name : string []) =
    Flyweight (level,name)
...
module Log =
  ...
  let create level (name : string) =
    if name = null then invalidArg "name" "name is null"
    Global.getStaticLogger (level,name.Split([|'.'|], StringSplitOptions.RemoveEmptyEntries))
    :> Logger

  /// Create an hierarchically named logger
  let createHiera level (name : string[]) =
    if name = null then invalidArg "name" "name is null"
    if name.Length = 0 then invalidArg "name" "must have >0 segments"
    Global.getStaticLogger (level ,name)
    :> Logger
...
module ProcessLogging =
  ...
  let mutable logger = Log.create LogLevel.Info "dotnet-mono"